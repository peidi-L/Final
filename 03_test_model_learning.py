...
def generate_synthetic_data(n_nodes=300, n_clusters=3):
    ...
    - Intra-cluster edges are mostly friends
    - Inter-cluster edges are mostly foes
    """
    ...
    # Sample edges based on cluster membership
    for i in range(n_nodes):
        for j in range(i + 1, n_nodes):
            ...
            if cluster_assign[i] == cluster_assign[j]:
                ...
                if rand_val < 0.3:  
                    # Mostly friends, some noise
                    sign = 1 if np.random.random() < 0.9 else -1
                    edges.append([i, j, sign])
            else:
                # Lower density between clusters
                if rand_val < 0.05:  
                    # Mostly foes, some noise
                    sign = -1 if np.random.random() < 0.9 else 1
                    edges.append([i, j, sign])
...
def spherical_distance(pos_i, pos_j):
    # Map points to unit sphere surface
...
def hyperbolic_distance(pos_i, pos_j):
    ...
    # Ensure points stay within the unit disk boundary
    ...
    # Distance formula for hyperbolic geometry
...
class SignedLSM_Simulation:
    ...
    def init_params(self):
        # Setup initial guesses for coordinates and parameters
    ...
    def neg_log_likelihood(self, params, edge_i, edge_j, signs):
        # Unpack the flattened parameter vector
        ...
        # Logit formulas for friendship and animosity
        logit_pos = alpha_pos - d
        logit_neg = alpha_neg + (beta * d)
        
        # Compute probabilities using soft-max normalization
        ...
        # Collect relevant probabilities based on observed data
    ...
    def predict(self, test_df):
        ...
        return exp_pos / partition, exp_neg / partition

# 4. Visualization and execution

if __name__ == "__main__":
    # Test model on three geometries
    models = [
